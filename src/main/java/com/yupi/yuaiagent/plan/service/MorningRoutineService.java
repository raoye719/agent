...
public class MorningRoutineService {
    ...
    private String buildMorningPrompt(LocalDate today, String weeklyPlan, String yesterdayProgress) {
        ...
        prompt.append("## 重要提示（必读）\n");
        prompt.append("1. 仔细阅读昨天的状态记录和备注\n");
        prompt.append("2. 如果昨天的状态记录中提到以下情况，则今天不布置任务：\n");
        prompt.append("   - \"用户明天要出去玩\"\n");
        prompt.append("   - \"用户明天有事\"\n");
        prompt.append("   - \"用户明天休息\"\n");
        prompt.append("   - 其他类似的休息/外出提示\n");
        prompt.append("3. 如果昨天的完成情况标记为\"作废\"或\"休息\"，则今天按原计划继续\n");
        prompt.append("4. 根据昨天的实际完成度灵活调整：\n");
        prompt.append("   - 完成度>80%：按原计划推进\n");
        prompt.append("   - 完成度50-80%：减少20%任务量\n");
        prompt.append("   - 完成度<50%：重复昨天未完成内容\n\n");
        
        prompt.append("## 任务\n");
        prompt.append("根据周计划和昨天的完成情况，为").append(today).append("生成具体的今日学习计划。\n\n");
        
        prompt.append("## 输出格式\n");
        prompt.append("### 情况1：正常学习日\n");
        ...
        prompt.append("### 情况2：休息日（如果昨天提到今天要休息）\n");
        prompt.append("# ").append(today.format(java.time.format.DateTimeFormatter.ofPattern("yyyy年M月d日")))
               .append(" 学习计划与完成情况\n\n");
        prompt.append("## 📅 今日学习计划\n");
        prompt.append("用户今天有事/要出去玩，不布置任务\n\n");
        prompt.append("## ✅ 今日完成情况\n");
        prompt.append("休息日，无任务\n\n");
        prompt.append("## 📝 状态记录\n");
        prompt.append("- 状态评价：休息\n");
        prompt.append("- 备注：用户有事，今天休息\n\n");
        prompt.append("## 🎯 明日建议\n");
        prompt.append("今天休息了，明天按原计划继续学习\n");
